...
URLS_GR11 = ["https://stanmorephysics.com/physical-science-grade-11/"]
URLS_GR10 = ["https://stanmorephysics.com/physical-science-grade-10/"]
...
def _get_priority_topic():
    ...
    try:
        ...
        # Filter for Physics only (Paper 2, Grade 11 for redox)
        physics_topics = [t for t in topics if t['subject'].lower() == 'physics']
        ...
        if r < 0.6 and lowest:
            ...
            print(f"[physics] PRIORITY: Lowest tier (0-40%) - {selected['topic']} ({selected['accuracy']:.0f}%)")
            return selected
        elif r < 0.9 and middle:
            selected = random.choice(middle)
            print(f"[physics] PRIORITY: Middle tier (40-70%) - {selected['topic']} ({selected['accuracy']:.0f}%)")
            return selected
        elif highest:
            selected = random.choice(highest)
            print(f"[physics] PRIORITY: Highest tier (70-100%) - {selected['topic']} ({selected['accuracy']:.0f}%)")
            return selected
    except Exception as e:
        print(f"[physics] Priority error: {e}")
    return None
...
def _generate_question_for_topic(topic_name):
    ...
    if "redox" in topic_lower or "oxidation" in topic_lower or "reduction" in topic_lower:
        questions = [
            ...
            {
                "question": "In the reaction: Zn + CuSO4 → ZnSO4 + Cu, identify what is oxidized and what is reduced.",
                "answer": "Zn is oxidized (loses electrons), Cu²⁺ is reduced (gains electrons).",
                "difficulty": "medium"
            },
            {
                "question": "Balance the redox reaction: MnO4⁻ + Fe²⁺ → Mn²⁺ + Fe³⁺ (in acidic medium).",
                "answer": "MnO4⁻ + 5Fe²⁺ + 8H⁺ → Mn²⁺ + 5Fe³⁺ + 4H₂O",
                "difficulty": "hard"
            },
            {
                "question": "What is a redox reaction? Give an example.",
                "answer": "A reaction where both oxidation and reduction occur. Example: 2Mg + O2 → 2MgO",
                "difficulty": "easy"
            }
        ]
    ...
    # Fallback to past paper if topic not in our generated list
    return _get_past_paper_question()

# ============================================
# PAST PAPER FUNCTIONS (unchanged)
# ============================================
...
def _is_actual_question(text):
    ...
    question_indicators = [
        ...
        r'A force of', r'A mass of', r'A car', r'A ball'
    ]
...
def _fetch_pdf_links(urls):
    ...
    for url in urls:
        try:
            ...
            print(f"[physics] fetch error {url}: {e}")
    print(f"[physics] found {len(links)} papers")
    return links
...
def _get_paper_chunks(url):
    ...
    try:
        ...
        print(f"[physics] {len(chunks)} question chunks from {url.split('/')[-1]}")
    except Exception as e:
        print(f"[physics] paper fetch error: {e}")
        _paper_cache[url] = []
...
def _get_past_paper_question():
    ...
    if not links:
        print("[physics] no papers found")
        return None

    label, url = random.choice(links)
    print(f"[physics] using past paper: {label}")

    chunks = _get_paper_chunks(url)
    if not chunks:
        print("[physics] no question chunks")
        return None
...
def get_question():
    ...
    print("[physics] No priority topics found, using past paper")
    return _get_past_paper_question()
